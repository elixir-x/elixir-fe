...
export const checkUsername = async (username: string): Promise<boolean> => {
	try {
		const response = await handleResponse(
			http.get(`/check-username?username=${username}`)
		);
...
export const updateUserInfo = async (
	props: Partial<UserUpdateProps>
): Promise<boolean> => {
	try {
		const formData = new FormData();

		if (props.profileBlob) formData.append("profile", props.profileBlob);
		if (props.username) formData.append("username", props.username);
		if (props.email) formData.append("email", props.email);
		if (props.bio) formData.append("bio", props.bio);
		if (props.password) formData.append("password", props.password);
