...
const onKeyUp = () => {
	...
	timer = setTimeout(async () => {
		...
		if (!success) setUsernameErrors("This username has been taken.");
...
const { value: email, errorMessage: emailError } = useField(
	...
	toFieldValidator(
		string({
			...
			invalid_type_error: "You must specify an email."
		})
...
const updateProfile = handleSubmit(async ({ bio }: Record<string, any>) => {
	successProfileModal.value = await updateUserInfo({
		bio,
		profileBlob: user.value?.profileBlob
	});
});

const updateAccount = handleSubmit(
	async ({
		username,
		email,
		password,
		...
		if (
			!password &&
			(username !== user.value?.username || email !== user.value?.email)
		) {
			setPasswordErrors(
				"You must enter a password to change important user details."
			);
			return;
		}
				...
				<form @submit.prevent="updateProfile" class="block space-y-2">
					<div class="flex items-center space-x-8">
						<ProfilePicturePreview
							...
							:change="true"
							class="w-[7.125rem] min-w-[7.125rem]" />
						<div>
							<div class="text-xl font-semibold text-white">
								{{ username }}
							</div>
							<p class="break-all text-sm">{{ bio }}</p>
						</div>
					</div>
					<InputWrapper name="bio" label="Bio" :error="bioError">
						<LimitedTextArea
							:max-length="300"
							height-class="h-[152px]"
							v-model="bio" />
					</InputWrapper>
